...
# fraom skfuzzy import control as ctrl
#may cause bug list:
...
def revert_names(type='str'):
    ...
    if Demir == 1:
        ...
        if type == 'str':
            return {'Pier 61 at Chelsea Piers': 0, 'West Drive Prospect Park West': 1,
                    'E 33 St 1 Ave': 2, 'Cleveland Pl Spring St': 3,
                    'N 7 St Driggs Ave': 4, 'West End Ave W 60 St': 5,
                    'N 6 St Bedford Ave': 6, 'Hanson Pl Ashland Pl': 7,
                    'Bergen St Vanderbilt Ave': 8,
                    'Franklin St Dupont St': 9,
                    }
        else:
...
# ========== 新增能量约束相关函数 (New energy constraint functions) ==========

def calculate_energy_consumption(k, distance_km, travel_time_hours):
    """
    计算能量消耗 (Calculate energy consumption)
    E^cons_{k,ij} = alpha_k * tau_ij + beta_k * d_ij (eq. 5)
    
    参数:
        k: 车辆索引 (vehicle index)
        distance_km: 距离 (公里) (distance in km)
        travel_time_hours: 旅行时间 (小时) (travel time in hours)
    
    返回:
        energy_consumption: 能量消耗 (kWh)
    """
    alpha_k = K[k, 10] if K.shape[1] > 10 else 1.0    # 时间能耗系数 (kWh/h)
    beta_k = K[k, 11] if K.shape[1] > 11 else 0.2     # 距离能耗系数 (kWh/km)
    
    energy_consumption = alpha_k * travel_time_hours + beta_k * distance_km
    return energy_consumption


def check_battery_feasibility(k, route):
    """
    检查路径的电池可行性 (Check battery feasibility of route)
    验证约束 (eq. 6, 7, 8):
    - e^k_j = e^k_i - E^cons_{k,ij} + ch^k_j  (energy balance)
    - 0 <= e^k_i <= B_k  (battery limits)
    - e^k_i >= E^cons_{k,ij} for flight  (flight feasibility)
    
    参数:
        k: 车辆索引 (vehicle index)
        route: 路径矩阵 (route matrix)
    
    返回:
        feasible: 布尔值,是否可行 (boolean, whether feasible)
        min_energy: 路径中最小电量 (minimum energy level in route)
    """
    B_k = K[k, 9] if K.shape[1] > 9 else 100.0        # 电池容量 (kWh)
    alpha_k = K[k, 10] if K.shape[1] > 10 else 1.0    # 时间能耗系数 (kWh/h)
    beta_k = K[k, 11] if K.shape[1] > 11 else 0.2     # 距离能耗系数 (kWh/km)
    
    remaining_energy = B_k  # 初始满电 (initial full charge)
    min_energy = B_k
    
    for x in range(1, len(route[4])):
        p1, d1 = int(route[0, x - 1]), int(route[0, x])
        if p1 == d1:
            continue
        ...
        if route[0, x] != route[0, x-1] and route[4, x - 1] != 'begin_depot':
            travel_time_hours = route[1, x] - route[3, x-1]
        
        # 计算能量消耗
        energy_consumption = alpha_k * travel_time_hours + beta_k * distance_km
        
        # 检查起飞可行性 (eq. 7)
        if remaining_energy < energy_consumption:
            return False, remaining_energy
        
        # 更新剩余电量 (eq. 6)
        remaining_energy -= energy_consumption
        
        # 充电策略: 当电量低于30%时充电至满
        if remaining_energy < B_k * 0.3:
            remaining_energy = B_k
        
        # 检查电池限制 (eq. 6)
        if remaining_energy < 0 or remaining_energy > B_k:
            return False, remaining_energy
        
        min_energy = min(min_energy, remaining_energy)
    
    return True, min_energy


def calculate_service_rate(routes_local):
    """
    计算服务率 (Calculate service rate, eq. 9)
    S = sum(z_r) / |R|
    
    参数:
        routes_local: 所有车辆的路径 (routes for all vehicles)
    
    返回:
        service_rate: 服务率 (service rate)
        served_count: 已服务的请求数 (number of served requests)
        total_count: 总请求数 (total number of requests)
    """
    served_requests = set()
    
    for k in range(len(K)):
        if len(routes_local[k][4]) > 2:
            for x in range(1, len(routes_local[k][4])):
                if hasNumbers(routes_local[k][4, x]):
                    request_number = int(''.join(filter(str.isdigit, routes_local[k][4, x])))
                    letters = new_getLetters(routes_local[k][4, x])
                    # 检查是否同时有pickup和delivery
                    if letters in ['pickup', 'delivery', 'Tp', 'Td', 'secondTp', 'secondTd']:
                        served_requests.add(request_number)
    
    served_count = len(served_requests)
    total_count = len(R) if len(R) > 0 else 1
    service_rate = served_count / total_count
    
    return service_rate, served_count, total_count


def validate_energy_constraints_global(routes_local):
    """
    全局验证所有车辆的能量约束 (Validate energy constraints for all vehicles globally)
    
    参数:
        routes_local: 所有车辆的路径 (routes for all vehicles)
    
    返回:
        all_feasible: 布尔值,所有路径是否可行 (boolean, whether all routes are feasible)
        infeasible_vehicles: 不可行的车辆列表 (list of infeasible vehicles)
    """
    infeasible_vehicles = []
    
    for k in range(len(K)):
        if len(routes_local[k][4]) > 2:
            feasible, min_energy = check_battery_feasibility(k, routes_local[k])
            if not feasible:
                infeasible_vehicles.append(k)
    
    all_feasible = len(infeasible_vehicles) == 0
    return all_feasible, infeasible_vehicles
...
def objective_value_k(k, new_try):
    ...
    # ---- 电量与租用成本参数 (Energy and rental cost parameters) ----
    # 从车辆参数矩阵K中读取,若列不存在则使用默认值
    B_k = K[k, 9] if K.shape[1] > 9 else 100.0        # 电池容量 Battery capacity (kWh)
    alpha_k = K[k, 10] if K.shape[1] > 10 else 1.0    # 时间能耗系数 Time-based energy consumption (kWh/h)
    beta_k = K[k, 11] if K.shape[1] > 11 else 0.2     # 距离能耗系数 Distance-based energy consumption (kWh/km)

    remaining_energy = B_k                             # 初始电量满 Initial battery fully charged
    total_operation_time = 0.0                        # 总运营时间 Total operation time (hours)
    rental_time_cost = 0.0                            # 租用时间成本 Rental time cost
    
    # 能量消耗记录 Energy consumption tracking
    energy_feasible = True                            # 路径能量可行性标志
    charging_energy = 0.0                             # 总充电量
    total_energy_consumption = 0.0                    # 总能量消耗
    ...
    for x in range(1, len(new_try[4])):
        if K[k, 5] != 3 or Demir == 1:
            ...
            if d == 1000000000:
                return 100000000000, 100000000000, 100000000000, 100000000000, 100000000000, 100000000000, 100000000000, 100000000000, 100000000000, 100000000000, 100000000000, 100000000000, 100000000000, 0, 0, 0

            distance = distance + d
            vehicle_cost = vehicle_cost + d * fuel_cost
            
            # ---- 计算旅行时间和能量消耗 (Calculate travel time and energy consumption) ----
            # 旅行时间 Travel time (hours)
            travel_time_hours = 0.0
            if new_try[0, x] != new_try[0, x-1] and new_try[4, x - 1] != 'begin_depot':
                travel_time_hours = new_try[1, x] - new_try[3, x-1]
                total_operation_time += travel_time_hours  # 累计运营时间
            
            # 能量消耗 E^cons_{k,ij} = alpha_k * tau_ij + beta_k * d_ij
            # d单位需要转换为km (假设D中存储的是米或其他单位,根据实际调整)
            distance_km = d / 1000.0  # 假设d是米,转换为公里
            energy_consumption = alpha_k * travel_time_hours + beta_k * distance_km
            total_energy_consumption += energy_consumption
            
            # ---- 电池约束检查 (Battery constraint check) ----
            # 检查当前电量是否足够完成此段行程 (eq. 7: flight feasibility)
            if remaining_energy < energy_consumption:
                # 电量不足,路径不可行 Insufficient energy, route infeasible
                energy_feasible = False
                # 返回一个很大的惩罚成本
                return 100000000000, 100000000000, 100000000000, 100000000000, 100000000000, 100000000000, 100000000000, 100000000000, 100000000000, 100000000000, 100000000000, 100000000000, 100000000000, 0, 0, 0
            
            # 更新剩余电量 (eq. 6: energy balance)
            # e^k_j = e^k_i - E^cons_{k,ij}
            remaining_energy -= energy_consumption
            
            # 在节点处充电 (假设所有节点都可以充电至满电)
            # 实际应用中可以根据节点类型决定是否充电
            # ch^k_j: charging energy at node j
            if remaining_energy < B_k * 0.3:  # 当电量低于30%时充电
                charge_amount = B_k - remaining_energy
                charging_energy += charge_amount
                remaining_energy = B_k
            
            # 电池容量限制检查 (eq. 6: 0 <= e^k_i <= B_k)
            if remaining_energy < 0 or remaining_energy > B_k:
                return 100000000000, 100000000000, 100000000000, 100000000000, 100000000000, 100000000000, 100000000000, 100000000000, 100000000000, 100000000000, 100000000000, 100000000000, 100000000000, 0, 0, 0
            ...
            if hasNumbers(new_try[4, x]):
                if new_try[0, x] == new_try[0, x-1] or new_try[4, x - 1] == 'begin_depot':
                    ...
                    # 添加服务时间到总运营时间 Add service time to total operation time
                    # 假设每个请求的服务时间可从R矩阵获取,或使用默认值
                    service_time = 0.1  # 默认服务时间(小时),可根据实际情况调整
                    total_operation_time += service_time
    ...
    # ---- 计算租用时间成本 (Calculate rental time cost, eq. 3) ----
    # C_rent_time = r_k * T_k
    rental_time_cost = rental_rate * total_operation_time
    
    # ---- 计算未服务请求成本 (Calculate unserved request cost, eq. 4) ----
    # C_unserved = sum(p^un_r * (1 - z_r))
    # 这里需要统计当前路径服务的请求数量
    served_requests_in_route = set()
    for x in range(1, len(new_try[4])):
        if hasNumbers(new_try[4, x]):
            request_number = int(''.join(filter(str.isdigit, new_try[4, x])))
            served_requests_in_route.add(request_number)
    ...
    if len(R) > 0:
        for idx in range(len(R)):
            request_id = R[idx, 7]
            if request_id not in served_requests_in_route:
                # p^un_r: 未服务请求的惩罚成本 (可以从R矩阵读取或设置默认值)
                penalty_unserved = R[idx, 8] * 2.0 if R.shape[1] > 8 else 100.0  # 默认惩罚
                unserved_cost += penalty_unserved
    ...
    # ---- 修改后的目标函数 (Modified objective function, eq. 1) ----
    ...
    # 定义权重 (可以设置为全局变量或从配置读取)
    # w1, w2, w3 已存在; 添加 w4, w5
    w4 = 0.1  # 租用时间成本权重 Rental time cost weight
    w5 = 0.05  # 未服务请求惩罚权重 Unserved request penalty weight
    ...
    if Demir == 1:
        ...
        cost = (w1 * (vehicle_cost + request_cost + wait_cost + transshipment_cost + un_load_cost)
                + w2 * delay_penalty
                + w3 * emission_cost
                + w4 * rental_time_cost
                + w5 * unserved_cost)
    ...
    # 返回值中增加新的成本组件
    # 原返回: cost, time, vehicle_cost, request_cost, wait_cost, transshipment_cost, 
    ...
    # 新增: rental_time_cost, unserved_cost, total_operation_time, remaining_energy, 
    #       total_energy_consumption, charging_energy
    
    return (round(cost, 3), round(time, 3), round(vehicle_cost, 3), round(request_cost, 3), 
            round(wait_cost, 3), round(transshipment_cost, 3), round(un_load_cost, 3), 
            round(distance, 3), round(profit, 3), round(emission, 3), round(emission_cost, 3), 
            round(storage_cost, 3), round(delay_penalty, 3), number_transshipment, 
            average_speed, average_time_ratio, round(rental_time_cost, 3), round(unserved_cost, 3),
            round(total_operation_time, 3), round(remaining_energy, 3), 
            round(total_energy_consumption, 3), round(charging_energy, 3))
...
def overall_obj(routes_local):
    ...
    if parallel_obj == 1:
        ...
        for k in range(len(K)):
            ...
            if len(routes_local[k][4]) > 2:
                cost, time, vehicle_cost, request_cost, wait_cost, transshipment_cost, un_load_cost, distance, profit, emission, emission_cost, storage_cost, delay_penalty, number_transshipment, average_speed, average_time_ratio, rental_time_cost, unserved_cost, total_operation_time, remaining_energy, total_energy_consumption, charging_energy = objective_value_k(k, routes_local[k])
